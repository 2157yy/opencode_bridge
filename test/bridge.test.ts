...
import { OpenCodeBridge, type BridgeSnapshot, BridgeRegistry, defaultLaunchPlan, mapSessionStatus } from '../src/index.js';
...
test('bridge starts, routes prompts, records artifacts, persists runtime metadata, and reuses llm config on restart', async () => {
  ...
  const statuses = new Map<string, { type: 'idle' | 'busy' | 'retry'; attempt?: number; message?: string; next?: number }>();
  const fakeClient = {
    session: {
      ...
      promptAsync: async ({ path: { id }, body }: { path: { id: string }; body: { parts: Array<{ type: 'text'; text: string }> } }) => {
  ...
  const bridge = new OpenCodeBridge({
    ...
    launcher: (command, args, options) => {
      ...
      return new FakeProcess(9000 + launches.length) as unknown as ChildProcess;
    },
  ...
  const persisted = JSON.parse(await readFile(statePath, 'utf8')) as BridgeSnapshot;
...
test('connectExisting reuses the saved backend URL when the existing runtime is still reachable', async () => {
  ...
  const statePath = join(projectDir, 'state.json');
  const savedSnapshot: BridgeSnapshot = {
    serverUrl: 'http://127.0.0.1:4096',
    projectDir,

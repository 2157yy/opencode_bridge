...
test('spawn command reuses the active runtime and forwards per-agent llm config', async () => {
  ...
  const fakeBridge: any = {
    ...
    async spawnAgent(options: Record<string, unknown>) {
...
test('spawn fails against inactive runtime without implicit recovery', async () => {
  ...
  await assert.rejects(
    () =>
      runCli(['spawn', '--project', '/tmp/project', '--name', 'researcher'], {
        createBridge: () => fakeBridge,
      }),
    /no active runtime/,
  );
...
test('route command reuses the active runtime without implicit startup or shutdown', async () => {
  ...
  const fakeBridge: any = {
    ...
    async route(options: Record<string, unknown>) {
      calls.push('route');
...
test('route fails against inactive runtime without implicit recovery', async () => {
  ...
  await assert.rejects(
    () =>
      runCli(['route', '--project', '/tmp/project', '--agent', 'session-2', '--text', 'inspect'], {
        createBridge: () => fakeBridge,
      }),
    /no active runtime/,
  );
...
test('restart fails against inactive runtime without implicit recovery', async () => {
  ...
  await assert.rejects(
    () =>
      runCli(['restart', '--project', '/tmp/project', '--agent', 'session-2'], {
        createBridge: () => fakeBridge,
      }),
    /no active runtime/,
  );
...
test('stop command preserves strict no-active-runtime failures', async () => {
  ...
  await assert.rejects(
    () =>
      runCli(['stop', '--project', '/tmp/project'], {
        createBridge: () => fakeBridge,
      }),
...
test('spawnAgent persists llm config in state and forwards it to the launch plan', async () => {
  ...
  const bridge = new OpenCodeBridge({
    ...
    launcher: () => new FakeProcess(7_001) as unknown as ChildProcess,
    launchPlanFactory: ((options: Record<string, unknown>) => {
  ...
  await bridge.start();
  const subagent = await (bridge as any).spawnAgent({
    name: 'researcher',
    apiKey: 'sk-test',
    baseUrl: 'https://llm.example.test',
    model: 'gpt-4o',
  ...
  const persisted = bridge.status().agents.find((agent) => agent.id === subagent.id) as any;
  const launch = launchOptions.at(-1) as any;
...
test('restartAgent reuses persisted llm config for relaunch', async () => {
  ...
  const bridge = new OpenCodeBridge({
    ...
    backendFactory: async () => ({ url: 'http://127.0.0.1:4096', close() {} }),
    clientFactory: () => fakeClient as never,
    launcher: () => new FakeProcess(8_001) as unknown as ChildProcess,
    launchPlanFactory: ((options: Record<string, unknown>) => {
      launchOptions.push(options);
      return {
        command: 'opencode',
  ...
  const relaunch = launchOptions[0] as any;
